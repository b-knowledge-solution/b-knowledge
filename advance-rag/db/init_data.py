...
Handles first-time setup of the application database, including:
- Creating a default superuser (admin) account with associated tenant
- Populating the LLM factory and model catalog from configuration
- Validating that default chat and embedding models are functional
- Fixing orphaned tenant_model_id references across tenants and knowledge bases

This module is typically invoked once during initial deployment or when
the database schema has been freshly created.
"""
import asyncio
import logging
import json
...
def init_superuser(nickname=DEFAULT_SUPERUSER_NICKNAME, email=DEFAULT_SUPERUSER_EMAIL, password=DEFAULT_SUPERUSER_PASSWORD, role=UserTenantRole.OWNER):
    """Create the default superuser account and associated tenant.

    If a user with the given email already exists, this function returns
    immediately without making changes. Otherwise it creates a user, tenant,
    user-tenant relationship, and registers the default LLM models for the
    tenant. It also validates that the configured chat and embedding models
    are functional by performing a quick test call.

    Args:
        nickname (str): Display name for the superuser.
        email (str): Email address for the superuser login.
        password (str): Initial password (will be base64-encoded before storage).
        role (UserTenantRole): Role to assign in the user-tenant relationship.
    """
    ...
    # Build the list of tenant LLM entries from the factory configuration
    ...
    # Validate that the configured chat model is reachable
    if tenant["llm_id"]:
    ...
    # Validate that the configured embedding model is reachable
    if tenant["embd_id"]:
...
def init_llm_factory():
    """Populate and synchronize the LLM factory and model catalog.

    This function performs the following:
    1. Clears all existing LLM factory entries and re-inserts from config
    2. Registers all model definitions under each factory
    3. Removes deprecated/legacy factory and model entries
    4. Updates tenant LLM entries with corrected factory names
    5. Resets parser_ids for all tenants to the canonical set
    6. Inserts missing OpenAI embedding models for existing OpenAI tenants
    7. Reconciles document counts across all knowledge bases
    """
    # Clear and re-populate the factory catalog from settings
    ...
    for factory_llm_info in factory_llm_infos:
        ...
        try:
            ...
            pass
        # Remove old model entries for this factory and re-insert
    ...
    # Clean up deprecated factories and models
    ...
    for tid in tenant_ids:
        for row in TenantLLMService.query(llm_factory="OpenAI", tenant_id=tid):
            ...
            try:
                ...
                pass
            break
    # Reconcile document counts for all knowledge bases
...
def init_web_data():
    """Initialize web application data.

    Orchestrates the full data initialization sequence: LLM factory catalog
    population and tenant model ID repair. Logs the total time taken.
    """
...
def fix_empty_tenant_model_id():
    """Repair NULL tenant_model_id references in knowledgebases and tenants.

    When a knowledgebase or tenant has a model name set but the corresponding
    tenant_llm row ID is NULL, this function looks up the correct tenant_llm
    entry and backfills the foreign key reference. This handles cases where
    data was created before the tenant_model_id column was introduced.
    """
    # Fix knowledgebase rows missing tenant_embd_id
    ...
    if empty_tenant_embd_id_kbs:
        ...
        # Group KBs by (tenant_id, embd_id) to minimize lookups
        kb_groups: dict = {}
    ...
    # Fix tenant rows missing tenant_model_id for various model types
    ...
    if empty_tenant_model_id_tenants:
        ...
        for obj in empty_tenant_model_id_tenants:
            ...
            update_dict = {}
            # Check each model type column and backfill if needed
