...
"""
Memory type utilities extracted from the deleted api/ layer.
"""
...
def format_ret_data_from_memory(memory):
    """Format a memory object into a serializable dict."""
    return {
        "id": memory.id,
        "name": memory.name,
...
def get_memory_type_human(memory_type: int) -> List[str]:
    """Convert a bitmask memory type to a list of human-readable names."""
...
def calculate_memory_type(memory_type_name_list: List[str]) -> int:
    """Convert a list of memory type names to a bitmask integer."""
    memory_type = 0
