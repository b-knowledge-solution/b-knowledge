/**
 * @fileoverview System history service for querying chat, search, and agent run session records.
 ...
 * All database access is delegated to SystemHistoryModel via ModelFactory.
 * @module services/system-history
 */
import { ModelFactory } from '@/shared/models/factory.js'
import { MessageRole } from '@/shared/constants/message-roles.js'

import type { FeedbackFilter } from '../models/system-history.model.js'
export type { FeedbackFilter } from '../models/system-history.model.js'
...
export class SystemHistoryService {
    ...
    async getChatHistory(
        ...
        const systemHistory = ModelFactory.systemHistory
        ...
        const externalBase = await systemHistory.findExternalChatSessions(filters)
        ...
        if (externalIds.length > 0) {
            const { firstPrompts, messageCounts, feedbackRows } = await systemHistory.findExternalChatEnrichmentData(externalIds, tenantId)
        ...
        const internalBase = await systemHistory.findInternalChatSessions(filters)
        ...
        if (internalIds.length > 0) {
            const { firstPrompts, messageCounts, feedbackRows } = await systemHistory.findInternalChatEnrichmentData(internalIds, tenantId)
    ...
    async getChatSessionDetails(sessionId: string, tenantId?: string) {
        ...
        const external = await systemHistory.findExternalChatMessages(sessionId)

        // Query feedback records for this chat session (tenant-scoped if provided)
        const feedbackRecords = await systemHistory.findFeedbackBySourceAndSession('chat', sessionId, tenantId)
        ...
        const messages = await systemHistory.findInternalChatMessages(sessionId)
    ...
    async getSearchHistory(
        ...
        const baseSessions = await systemHistory.findSearchSessions(filters)
        if (baseSessions.length === 0) return []

        const sessionIds = baseSessions.map((row: any) => row.session_id)
        const { firstInputs, messageCounts, feedbackRows } = await systemHistory.findSearchEnrichmentData(sessionIds, tenantId)
    ...
    async getSearchSessionDetails(sessionId: string, tenantId?: string) {
        ...
        const records = await systemHistory.findSearchRecords(sessionId)

        // Query feedback records for this search session (tenant-scoped if provided)
        const feedbackRecords = await systemHistory.findFeedbackBySourceAndSession('search', sessionId, tenantId)
    ...
    async getAgentRunHistory(
        ...
        const runs = await systemHistory.findAgentRuns(filters)
        if (runs.length === 0) return []

        const runIds = runs.map((r: any) => r.run_id)
        const feedbackRows = await systemHistory.findAgentRunFeedback(runIds, tenantId)
    ...
    async getAgentRunDetails(runId: string, tenantId?: string) {
        ...
        const run = await systemHistory.findAgentRunWithDetails(runId, tenantId)

        if (!run) return null

        // Fetch run steps and feedback in parallel
        const [steps, feedback] = await Promise.all([
            systemHistory.findAgentRunSteps(runId),
            systemHistory.findAgentRunDetailFeedback(runId, tenantId),
        ])
    ...
    async getSystemChatHistory(
        ...
        return ModelFactory.systemHistory.findSystemChatSessionsWithMessages(page, limit, search)
    }
}

/** Singleton instance of SystemHistoryService */
export const systemHistoryService = new SystemHistoryService();
