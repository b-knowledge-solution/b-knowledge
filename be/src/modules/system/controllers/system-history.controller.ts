/**
 * @fileoverview System History Controller
...
import { systemHistoryService } from '@/modules/system/services/system-history.service.js';
import type { FeedbackFilter } from '@/modules/system/services/system-history.service.js';

/**
 * @description Handles system requests for viewing chat, search, and agent run history across all users
 */
export class SystemHistoryController {
    ...
    async getChatHistory(req: Request, res: Response): Promise<void> {
        try {
            ...
            const sessions = await systemHistoryService.getChatHistory(page, limit, search, email, startDate, endDate, sourceName, feedbackFilter, tenantId);
            res.json(sessions);
        } catch (error) {
    ...
    async getChatSessionDetails(req: Request, res: Response): Promise<void> {
        try {
            ...
            const messages = await systemHistoryService.getChatSessionDetails(sessionId, tenantId);
            res.json(messages);
        } catch (error) {
            // Log error and return 500 status
    ...
    async getSearchHistory(req: Request, res: Response): Promise<void> {
        try {
            ...
            const sessions = await systemHistoryService.getSearchHistory(page, limit, search, email, startDate, endDate, sourceName, feedbackFilter, tenantId);
            res.json(sessions);
        } catch (error) {
            // Log error and return 500 status
    ...
    async getSearchSessionDetails(req: Request, res: Response): Promise<void> {
        try {
            ...
            const messages = await systemHistoryService.getSearchSessionDetails(sessionId, tenantId);
            res.json(messages);
        } catch (error) {
            // Log error and return 500 status
    ...
    async getAgentRunHistory(req: Request, res: Response): Promise<void> {
        try {
            ...
            const runs = await systemHistoryService.getAgentRunHistory(page, limit, search, email, startDate, endDate, feedbackFilter, tenantId)
            res.json(runs)
        } catch (error) {
    ...
    async getAgentRunDetails(req: Request, res: Response): Promise<void> {
        try {
            ...
            const details = await systemHistoryService.getAgentRunDetails(runId, tenantId)
    ...
    async getSystemChatHistory(req: Request, res: Response): Promise<void> {
        try {
            ...
            const history = await systemHistoryService.getSystemChatHistory(page, limit, search);
            res.json(history);
        } catch (error) {
            // Log error and return 500 status
