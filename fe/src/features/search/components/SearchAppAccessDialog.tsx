/**
 * @fileoverview Dialog for managing user and team access to a search app.
...
interface SearchAppAccessDialogProps {
  ...
  onClose: () => void
  /** The search app to manage access for */
  app: SearchApp | null
...
/**
 * @description Modal dialog for managing user and team access to a search app.
 * Displays tabs for Users and Teams with searchable checkbox lists.
 *
 * @param {SearchAppAccessDialogProps} props - Component properties
 * @returns {JSX.Element} The rendered access management dialog
 */
export default function SearchAppAccessDialog({
  open,
  onClose,
  app,
  onSave,
}: SearchAppAccessDialogProps) {
  const { t } = useTranslation()

  // Current access entries from the API (used to initialize selected sets)
  const [, setAccessEntries] = useState<SearchAppAccessEntry[]>([])
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const entries = await searchApi.getSearchAppAccess(app.id)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const entries: SearchAppAccessEntry[] = isPublic ? [] : [
      ...
      await Promise.all([
        searchApi.setSearchAppAccess(app.id, entries),
        isPublic !== app.is_public ? searchApi.updateSearchApp(app.id, { is_public: isPublic }) : Promise.resolve(),
      ])
  ...
  return (
    <Dialog open={open && !!app} onOpenChange={(v: boolean) => { if (!v) onClose() }}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader className="mb-2">
          <DialogTitle>
            {t('searchAdmin.manageAccess')} - {app?.name}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex items-center justify-between mb-4 pb-4 border-b">
          <div>
            <p className="text-sm font-medium">{t('searchAdmin.isPublic')}</p>
