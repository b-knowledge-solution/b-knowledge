 ...
 *
 * @module features/knowledge-base/components/SyncTab
 */
...
const SyncTab = ({ knowledgeBaseId }: SyncTabProps) => {
  ...
  const fetchConfigs = async () => {
    try {
      setLoading(true)
      const data = await getSyncConfigs(knowledgeBaseId)
      setConfigs(data)
    } catch (err) {
  ...
  const handleSaveConfig = async (data: {
    ...
    try {
      if (activeConfig) {
        await updateSyncConfig(knowledgeBaseId, activeConfig.id, data)
      } else {
        await createSyncConfig(knowledgeBaseId, data)
      }
      await fetchConfigs()
    } finally {
      setSaving(false)
  ...
  const handleSaveSchedule = async (schedule: string) => {
    ...
    setSaving(true)
    try {
      await updateSyncConfig(knowledgeBaseId, activeConfig.id, { sync_schedule: schedule })
      await fetchConfigs()
    } finally {
      setSaving(false)
    }
