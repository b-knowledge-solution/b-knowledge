 ...
 * @module features/knowledge-base/components/PipelineStatusBar
 */

import { useState, useEffect } from 'react'
import { useTranslation } from 'react-i18next'
import { FileCode2, Network, Cpu, CheckCircle2, XCircle, Circle } from 'lucide-react'
import { syncVersionParserStatus } from '../api/knowledgeBaseApi'
 ...
 */
export default function PipelineStatusBar({ datasetId, knowledgeBaseId, categoryId }: PipelineStatusBarProps) {
  ...
  useEffect(() => {
    ...
    if (!knowledgeBaseId || !categoryId || !datasetId) return

    let cancelled = false

    const pollStatus = async () => {
      try {
        const statuses = await syncVersionParserStatus(knowledgeBaseId, categoryId, datasetId)
  ...
  }, [knowledgeBaseId, categoryId, datasetId])
