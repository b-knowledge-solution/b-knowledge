...
const DatasetCard: React.FC<DatasetCardProps> = ({ dataset, onEdit, onDelete, isAdmin }) => {
  ...
  return (
    <Card
      hoverable
      className="dark:bg-slate-800 dark:border-slate-700 cursor-pointer group"
    ...
    >
      ...
      <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
        <Tooltip title={t('datasets.docCount')}>
          <span className="flex items-center gap-1">
            <FileText size={14} />
            {dataset.doc_count}
          </span>
        </Tooltip>
        <Tooltip title={t('datasets.chunkCount')}>
          <span className="flex items-center gap-1">
            <Hash size={14} />
            {dataset.chunk_count}
          </span>
        </Tooltip>
        <Tooltip title={dataset.access_control?.public ? t('datasets.public') : t('datasets.private')}>
          <span className="flex items-center gap-1">
