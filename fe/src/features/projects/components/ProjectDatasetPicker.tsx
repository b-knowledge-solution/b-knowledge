/**
 * @fileoverview Project dataset picker for binding/unbinding datasets.
 * Shows bound datasets in a table and provides a multi-select dialog for binding new ones.
 * @module features/projects/components/ProjectDatasetPicker
 */
...
import { useProjectBoundDatasets, useBindDatasets, useUnbindDataset } from '../api/projectQueries'

// ============================================================================
// Types
// ============================================================================

interface ProjectDatasetPickerProps {
  /** Project UUID to manage dataset bindings for */
  projectId: string
}
 ...
 * @param {ProjectDatasetPickerProps} props - Component props
 * @returns {JSX.Element} Rendered dataset picker with bind/unbind functionality
 */
export default function ProjectDatasetPicker({ projectId }: ProjectDatasetPickerProps) {
  const { t } = useTranslation()
  const confirm = useConfirm()

  // Data hooks
  const { data: boundDatasets = [], isLoading } = useProjectBoundDatasets(projectId)
  const bindDatasets = useBindDatasets(projectId)
  const unbindDataset = useUnbindDataset(projectId)
