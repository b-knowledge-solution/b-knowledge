 ...
 *
 * @description Launches the converter worker via .venv Python or falls back to system Python.
...
function resolvePython() {
  ...
  if (!existsSync(join(rootDir, '.venv'))) {
    console.error('')
    console.error('\x1b[31m[run-converter] ERROR: Python virtual environment not found.\x1b[0m')
    console.error('')
  ...
  if (!isPythonWorking(venvPython)) {
    console.error('')
    console.error('\x1b[31m[run-converter] ERROR: Python in .venv is broken (base Python may have been uninstalled).\x1b[0m')
    console.error('')
...
console.log(`[run-converter] Starting converter worker with ${python}...`)
